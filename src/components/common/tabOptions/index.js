...
const TabOptions = ({activeTab, setActiveTab}) => {
  ...
  <div className="tab-options">
    <div className="max-width options-wrapper">
      {tabs.map((tab) =>{
        return <div onClick={()=> setActiveTab(tab.name)} className={`tab-item absolute-center cu-po ${activeTab===tab.name && "active-tab"}`}>

          <div className='tab-image-container absolute-center'
          style={{backgroundColor: `${activeTab===tab.name?tab.backdrop: ""}`}}>

            <img className="tab-image" alt={tab.name} sec={ activeTab===tab.name ? tab.active_img:tab.inactive_img} />
          </div>
